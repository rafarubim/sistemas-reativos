...
function Object:_updateKeyPressed(dt)
  for control in Controls:values() do
    local key = self.controls[control]
    if key and love.keyboard.isDown(key) and not self.handlersTriggered[control] then
      self.handlersTriggered[control] = true
      local handler = self._controlHandlers[control]
      if handler then
        handler(self, control)
      end
    elseif key and not love.keyboard.isDown(key) and self.handlersTriggered[control] then
      self.handlersTriggered[control] = false
      local handler = self._controlHandlers[control]
      if handler then
        handler(self, control, true)
      end
    end
  end
end
